...
export function App() {
  ...
  async function updateClass(id: string, payload: Partial<ClassInput>) {
    if (!auth || !profile?.isAdmin) return;
    await run(async () => { await portalApi.updateClass(id, payload, auth); await Promise.all([refreshAcademicData(), refreshAdminData()]); }, "Class updated.");
  }
...
function ClassEditorCard({ item, updateClass, loading }: { item: ClassSession; updateClass: (id: string, payload: Partial<ClassInput>) => Promise<void>; loading: boolean }) {
  const [draft, setDraft] = useState<ClassInput>({ courseId: item.courseId, courseTitle: item.courseTitle, title: item.title, schedule: item.schedule, modality: item.modality, instructor: item.instructor, seats: item.seats, status: item.status });
  async function submit(event: FormEvent<HTMLFormElement>) {
    event.preventDefault();
    await updateClass(item.id, draft);
  }
  return <article className="card compact"><div className="card-title"><h3>{item.courseTitle}</h3><span className="pill">{item.enrolled}/{item.seats}</span></div><form className="inline-fields" onSubmit={submit}><input value={draft.title} onChange={(event) => setDraft({ ...draft, title: event.target.value })} aria-label="Class title" /><input value={draft.schedule} onChange={(event) => setDraft({ ...draft, schedule: event.target.value })} aria-label="Schedule" /><select value={draft.modality} onChange={(event) => setDraft({ ...draft, modality: event.target.value })} aria-label="Modality"><option>Hybrid</option><option>Remote</option><option>On campus</option></select><input value={draft.instructor} onChange={(event) => setDraft({ ...draft, instructor: event.target.value })} aria-label="Instructor" placeholder="Instructor" /><input type="number" min="1" value={draft.seats} onChange={(event) => setDraft({ ...draft, seats: Number(event.target.value) })} aria-label="Seats" /><select value={draft.status} onChange={(event) => setDraft({ ...draft, status: event.target.value as ClassInput["status"] })} aria-label="Status"><option>Scheduled</option><option>Open</option><option>Full</option><option>InProgress</option><option>Complete</option><option>Cancelled</option></select><button disabled={loading}>Update</button></form></article>;
}
